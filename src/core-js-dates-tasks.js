...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const resultMilliseconds = end.getTime() - start.getTime();

  const resultDays = Math.floor(resultMilliseconds / (1000 * 60 * 60 * 24)) + 1;
...
function getCountWeekendsInMonth(month, year) {
  ...
  const indexTargetMonth = month - 1;

  const daysInMonth = new Date(year, month, 0).getDate();

  for (let day = 1; day <= daysInMonth; day += 1) {
    const date = new Date(year, indexTargetMonth, day);
...
function getWeekNumberByDate(date) {
  ...
  const startYear = new Date(Date.UTC(targetDate.getUTCFullYear(), 0, 1));

  const week = Math.ceil(((targetDate - startYear) / 86400000 + 1) / 7);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      result.push(formatWorkDate(currentDate));
